...
function highlightActiveLyric(index, isUserClick = false) {
  ...
  // Si la línea ya está activa y no es un clic/forzado, salimos inmediatamente para evitar loops de scroll y bloqueos
  if (activeLine.classList.contains("active") && !isUserClick) return;
  ...
  // Guardamos la marca de tiempo del scroll programático para que el listener ignore los eventos generados por smooth scroll
...
function initLyricsScrollListener() {
  ...
  container.addEventListener("scroll", () => {
    // Si el scroll ocurre dentro de la ventana de animación del scroll automático (800ms), se ignora
